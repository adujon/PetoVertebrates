...
#fit the phylogenetic regression model using brms
fit_logmass_malignant_zi <- brm(NMalignant | trials(NNecropsies) ~ logMass + (1|gr(Species, cov = vcv_brms)),
...
#export the model
setwd(export_dir)
saveRDS(fit_logmass_malignant, "Vincze_CMR_mammals_malignant_logMass.rds")
...
#export the model
setwd(export_dir)
saveRDS(fit_logMaxLongevity_malignant, "Vincze_CMR_mammals_malignant_logMaxLongevity.rds")
