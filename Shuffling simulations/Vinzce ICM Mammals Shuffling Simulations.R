...
write.csv(simul_df, "Vincze ICM Mammals Mass.csv", row.names = F)
...
for(i in 1:n_simul){
  ...
  #append the dataframe with the results
  res_df <- data.frame(Data = "Vincze et al.", TumorType = "Malignant", RiskFactor = "Longevity", Simualtion = i, Estimate = res[2,1], U95CI = res[2,3],  L95CI = res[2,4])
  simul_df <- rbind(simul_df, res_df)
  
}

setwd("C:/Users/antoi/My Drive/Postdoc/Zoo data/Peto/scripts/Binomial models scripts/Shuffling simulations/Results")
write.csv(simul_df, "Vincze ICM Mammals Longevity.csv", row.names = F)
