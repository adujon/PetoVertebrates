...
#subset the data to only keep species with cancer
data <- subset(data, data$ICM>0)
...
simul_df <- data.frame(Data = NULL, TumorType = NULL, RiskFactor = NULL, Simulation = NULL, Estimate = NULL, L95CI = NULL, U95CI = NULL, pvalue = NULL)
...
for(i in 1:n_simul){
  ...
  #logit transform the prevalences
  data_simul$ICMlogit <- car::logit(data_simul$ICM)
  ...
  fit_logmass_malignant<- gls(ICMlogit ~  logMass, data =data_simul,
                          cor=corPagel(0,phylo_all , form = ~Species, fixed=F), weights = ~1/log(NNecropsies))
  
  #get the effect sizes for the current model
  res <- summary(fit_logmass_malignant)
  
  #append the dataframe with the results
  res_df <- data.frame(Data = "Vincze et al.", TumorType = "Malignant", RiskFactor = "Mass", Simulation = i, Estimate = res$tTable[2,1], U95CI = res$tTable[2,1] + 1.96 * res$tTable[2,2],  L95CI = res$tTable[2,1] - 1.96 * res$tTable[2,2], pvalue =  res$tTable[2,4])
...
write.csv(simul_df, "Vincze ICM Mammals Mass Gaussian Model.csv", row.names = F)
...
for(i in 1:n_simul){
  ...
  #remove NAs
  data_simul <- subset(data_simul, is.na(data_simul$logMaxLongevity) == F)
  ...
  #fit the phylogenetic regression model using brms
  fit_logmass_malignant<- gls(ICMlogit ~  logMaxLongevity, data =data_simul,
                              cor=corPagel(0,phylo_all , form = ~Species, fixed=F), weights = ~1/log(NNecropsies))
  ...
  #append the dataframe with the results
  res_df <- data.frame(Data = "Vincze et al.", TumorType = "Malignant", RiskFactor = "logMaxLongevity", Simulation = i, Estimate = res$tTable[2,1], U95CI = res$tTable[2,1] + 1.96 * res$tTable[2,2],  L95CI = res$tTable[2,1] - 1.96 * res$tTable[2,2], pvalue =  res$tTable[2,4])
  simul_df <- rbind(simul_df, res_df)
  
}

setwd("C:/Users/antoi/My Drive/Postdoc/Zoo data/Peto/scripts/Binomial models scripts/Shuffling simulations/Results")
write.csv(simul_df, "Vincze ICM Mammals Longevity Gaussian Model.csv", row.names = F)
